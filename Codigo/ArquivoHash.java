...
// CLASSE DO ARQUIVO DE HASH
public class ArquivoHash{
    ...
    // CONSTRUTOR DO ARQUIVO HASH
    public ArquivoHash(String nomeArquivo) throws Exception{
    ...
    // METODO PARA FAZER A LEITURA DO CSV DE PILOTOS (HASH EXTENSIVEL) (REUTILIZADO DO TP02 DE AEDS 02)
    ...
    public int adicionarPiloto(Piloto piloto) throws Exception{
        ...
        // CRIA O INDICE
        indiceDireto.adicionaHash(new ParIDEndereco(proximoID, endereco));
    ...
    public Piloto buscarPiloto(int id) throws Exception{
        ...
        ParIDEndereco idEndereco = indiceDireto.buscarHash(id);
        if(idEndereco != null){
            // PULA PARA O ENDERECO
            arquivo.seek(idEndereco.getEndereco());
            piloto = new Piloto();
    ...
    public boolean excluirPiloto(int id) throws Exception{
        ...
        if(idEndereco != null){
            ...
            if(lapide == 1){
                ...
                if(piloto.getId() == id){
                    // EXCLUI O INDICE
                    if(indiceDireto.excluirHash(id)){
                        arquivo.seek(idEndereco.getEndereco());
                        // ESCREVE LAPIDE COMO EXCLUIDO
                        arquivo.write(0);
                        addDeleted(tam, idEndereco.getEndereco());
                        return true;
                    }
                }
            }
    ...
    public boolean atualizarPiloto(Piloto novopiloto) throws Exception{
        ...
        byte lapide;
        // PEGA O ENDERECO NO INDICE
        ParIDEndereco idEndereco = indiceDireto.buscarHash(novopiloto.getId());
        ...
        if(idEndereco != null){
            ...
            if(lapide == 1){
                ...
                if(piloto.getId() == novopiloto.getId()){
                    ...
                    if(tam2 <= tam){
                        arquivo.seek(idEndereco.getEndereco()+3);
                        arquivo.write(b2);
                    }
                    // SE NAO ESCREVE O FIM DO ARQUIVO
                        ...
                        // ATUALIZA O INDICE
                        indiceDireto.update(new ParIDEndereco(novopiloto.getId(), novoEndereco));
                    }
                    return true;
                }
            }
